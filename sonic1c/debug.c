...
ROMFUNC(rom_1D6E6) {
  ...
  DEF_ROMLOC(1D6E8) : move_toreg_8(read_8(v_debuguse), &D0);
  switchindex = D0 / 2; // MOVE.B	$FE08,D0
  DEF_ROMLOC(1D6EC)
      : move_toreg_16(read_16(0x1D6F4 + (s16)(D0 & 0xffff)),
                      &D1); // MOVE.W	6(PC,D0),D1
  // DEF_ROMLOC(1D6F0): goto rom_1D6F4 + (s16)(D1 & 0xffff); // JMP	2(PC,D1)
  DEF_ROMLOC(1D6F0) : switch (switchindex) {
  case 0:
    rom_1D6F8();
    break;
  case 1:
...
ROMFUNC(rom_1D6F8) {
  DEF_ROMLOC(1D6F8) : add_tomem_8(0x2, v_debuguse); // ADDQ.B	#$02,$FE08
  DEF_ROMLOC(1D6FC)
      : move_tomem_16(read_16(0xFFFFF72C),
                      0xFFFFFEF0); // MOVE.W	$F72C,$FEF0
  DEF_ROMLOC(1D702)
      : move_tomem_16(read_16(0xFFFFF726),
                      0xFFFFFEF2); // MOVE.W	$F726,$FEF2
  DEF_ROMLOC(1D708)
  ...
  DEF_ROMLOC(1D70E)
      : move_tomem_16(0x720, 0xFFFFF726); // MOVE.W
                                          // #$0720,$F726
  DEF_ROMLOC(1D714)
      : and_tomem_16(0x7FF, 0xFFFFD00C); // ANDI.W
                                         // #$07FF,$D00C
  DEF_ROMLOC(1D71A)
      : and_tomem_16(0x7FF, 0xFFFFF704); // ANDI.W
                                         // #$07FF,$F704
  DEF_ROMLOC(1D720)
      : and_tomem_16(0x3FF, 0xFFFFF70C);            // ANDI.W
                                                    // #$03FF,$F70C
  DEF_ROMLOC(1D726) : move_tomem_8(0x0, A0 + 0x1A); // MOVE.B	#$00,26(A0)
  DEF_ROMLOC(1D72C) : move_tomem_8(0x0, A0 + 0x1C); // MOVE.B	#$00,28(A0)
  DEF_ROMLOC(1D738) : if (v_gamemode != GM_SPECIAL) goto rom_1D74A; // BNE.B	$1D74A
  DEF_ROMLOC(1D73A)
      : move_tomem_16(0x0, 0xFFFFF782); // MOVE.W	#$0000,$F782
  DEF_ROMLOC(1D740)
      : move_tomem_16(0x0, 0xFFFFF780);                  // MOVE.W	#$0000,$F780
  ...
  DEF_ROMLOC(1D74C) : move_toreg_8(read_8(v_zone), &D0); // MOVE.B	$FE10,D0
...
ROMFUNC(rom_1D77A) {
  ...
  DEF_ROMLOC(1D782) : if (v_gamemode == GM_SPECIAL) goto rom_1D78A;        // BEQ.B	$1D78A
  DEF_ROMLOC(1D784) : move_toreg_32(0x0, &D0);           // MOVEQ.L	$00,D0
  DEF_ROMLOC(1D786) : move_toreg_8(read_8(v_zone), &D0); // MOVE.B	$FE10,D0
...
ROMFUNC(rom_1D7A2) {
  ...
  DEF_ROMLOC(1D7DE)
      : move_tomem_8(0xFFFFFFFF, 0xFFFFFE0B); // MOVE.B
                                              // #$FF,$FE0B
  DEF_ROMLOC(1D7E4)
      : move_toreg_8(read_8(0xFFFFF604), &D4); // MOVE.B	$F604,D4
  ...
  DEF_ROMLOC(1D88E)
      : move_tomem_8(read_8(A0 + 0x4), A1 + 0x0); // MOVE.B	4(A0),0(A1)
  DEF_ROMLOC(1D894)
      : move_tomem_8(read_8(A0 + 0x1), A1 + 0x1); // MOVE.B	1(A0),1(A1)
  DEF_ROMLOC(1D89A)
      : move_tomem_8(read_8(A0 + 0x1), A1 + 0x22);  // MOVE.B	1(A0),34(A1)
  DEF_ROMLOC(1D8A0) : and_tomem_8(0x7F, A1 + 0x22); // ANDI.B	#$7F,34(A1)
  DEF_ROMLOC(1D8A6) : move_toreg_32(0x0, &D0);      // MOVEQ.L	$00,D0
  DEF_ROMLOC(1D8A8)
      : move_toreg_8(read_8(0xFFFFFE06), &D0); // MOVE.B	$FE06,D0
  DEF_ROMLOC(1D8AC) : lsl_toreg_16(0x3, &D0);  // LSL.W	#$03,D0
  DEF_ROMLOC(1D8AE)
      : move_tomem_8(read_8((s32)A2 + (s8)0x4 + (s16)D0),
                     A1 + 0x28);                     // MOVE.B	4(A2,D0),40(A1)
  ...
  DEF_ROMLOC(1D8C0) : move_tomem_16(D0, v_debuguse); // MOVE.W	D0,$FE08
  DEF_ROMLOC(1D8C4)
      : move_tomem_32(0x21CF4, 0xFFFFD004); // MOVE.L	#$00021CF4,$D004
  DEF_ROMLOC(1D8CC)
      : move_tomem_16(0x780, 0xFFFFD002);           // MOVE.W
                                                    // #$0780,$D002
  ...
  DEF_ROMLOC(1D8DE)
      : move_tomem_16(read_16(0xFFFFFEF0),
                      0xFFFFF72C); // MOVE.W	$FEF0,$F72C
  DEF_ROMLOC(1D8E4)
      : move_tomem_16(read_16(0xFFFFFEF2),
                      0xFFFFF726); // MOVE.W	$FEF2,$F726
  DEF_ROMLOC(1D8F0) : if (v_gamemode != GM_SPECIAL) goto rom_1D91C; // BNE.B	$1D91C
  DEF_ROMLOC(1D8F2) : clr_mem_16(0xFFFFF780);      // CLR.W	$F780
  DEF_ROMLOC(1D8F6)
      : move_tomem_16(0x40, 0xFFFFF782); // MOVE.W
                                         // #$0040,$F782
  DEF_ROMLOC(1D8FC)
      : move_tomem_32(0x21CF4, 0xFFFFD004); // MOVE.L	#$00021CF4,$D004
  DEF_ROMLOC(1D904)
