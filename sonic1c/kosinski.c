...
ROMFUNC(rom_1894) {
  ...
  DEF_ROMLOC(1896)
      : move_tomem_8(read_8(A0++), A7 + 0x1);         // MOVE.B	(A0)+,1(A7)
  ...
  DEF_ROMLOC(18A4) : dec_reg_16(&D4);
  if ((D4 & 0xffff) != 0xffff)
    goto rom_18B2; // DBF.W	D4,$18B2
  DEF_ROMLOC(18A8)
      : move_tomem_8(read_8(A0++), A7 + 0x1);          // MOVE.B	(A0)+,1(A7)
  ...
  DEF_ROMLOC(18B2) : move_toccr_8(D6, &SR);            // MOVE.W	D6,CCR
  DEF_ROMLOC(18B4) : if (CCR_CC) goto rom_18BA;        // BCC.B	$18BA
  DEF_ROMLOC(18B6) : move_tomem_8(read_8(A0++), A1++); // MOVE.B
                                                       // (A0)+,(A1)+
  ...
  DEF_ROMLOC(18C0) : dec_reg_16(&D4);
  if ((D4 & 0xffff) != 0xffff)
    goto rom_18CE; // DBF.W	D4,$18CE
  DEF_ROMLOC(18C4)
      : move_tomem_8(read_8(A0++), A7 + 0x1);         // MOVE.B	(A0)+,1(A7)
  ...
  DEF_ROMLOC(18CE) : move_toccr_8(D6, &SR);           // MOVE.W	D6,CCR
  DEF_ROMLOC(18D0) : if (CCR_CS) goto rom_18FE;       // BCS.B	$18FE
  DEF_ROMLOC(18D2) : lsr_toreg_16(0x1, &D5);          // LSR.W	#$01,D5
  DEF_ROMLOC(18D4) : dec_reg_16(&D4);
  if ((D4 & 0xffff) != 0xffff)
    goto rom_18E2; // DBF.W	D4,$18E2
  DEF_ROMLOC(18D8)
      : move_tomem_8(read_8(A0++), A7 + 0x1);         // MOVE.B	(A0)+,1(A7)
  ...
  DEF_ROMLOC(18E6) : dec_reg_16(&D4);
  if ((D4 & 0xffff) != 0xffff)
    goto rom_18F4; // DBF.W	D4,$18F4
  DEF_ROMLOC(18EA)
      : move_tomem_8(read_8(A0++), A7 + 0x1);         // MOVE.B	(A0)+,1(A7)
  ...
  DEF_ROMLOC(1914)
      : move_toreg_8(read_8((s32)A1 + (s8)0x0 + (s16)D2),
                     &D0);                   // MOVE.B	0(A1,D2),D0
  DEF_ROMLOC(1918) : move_tomem_8(D0, A1++); // MOVE.B	D0,(A1)+
  DEF_ROMLOC(191A) : dec_reg_16(&D3);
  if ((D3 & 0xffff) != 0xffff)
    goto rom_1914;                                    // DBF.W	D3,$1914
