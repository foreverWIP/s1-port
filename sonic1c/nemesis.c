...
static void NemDec_ProcessCompressedData(bool skiptonewrow) {
  ...
  DEF_ROMLOC(14C8) : dec_reg_16(&D0);
  if ((D0 & 0xffff) != 0xffff)
    goto rom_14BA;                                    // DBF.W	D0,$14BA
  ...
  DEF_ROMLOC(1510)
      : move_tomem_32(D4, (A4 += 4, A4 - 4));    // MOVE.L	D4,(A4)+
  ...
  DEF_ROMLOC(151C)
      : move_tomem_32(D2, (A4 += 4, A4 - 4));    // MOVE.L	D2,(A4)+
...
ROMFUNC(rom_14C4) { NemDec_ProcessCompressedData(true); }
ROMFUNC(rom_1482) { NemDec_ProcessCompressedData(false); }
ROMFUNC(rom_1526) {
  ...
  DEF_ROMLOC(155A)
      : move_tomem_16(D7,
                      (s32)A1 + (s8)0x0 + (s16)D0);   // MOVE.W	D7,0(A1,D0)
  ...
  DEF_ROMLOC(1572) : dec_reg_16(&D5);
  if ((D5 & 0xffff) != 0xffff)
    goto rom_156C; // DBF.W	D5,$156C
  goto rom_1532;   // BRA.B	$1532
}