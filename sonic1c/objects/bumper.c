...
ROMFUNC(rom_F05C) {
  ...
  DEF_ROMLOC(F05E) : move_toreg_8(read_8(A0 + 0x24), &D0);
  switchindex = D0 / 2; // MOVE.B	36(A0),D0
  DEF_ROMLOC(F062)
      : move_toreg_16(read_16(0xF06A + (s16)(D0 & 0xffff)),
                      &D1); // MOVE.W	6(PC,D0),D1
  // DEF_ROMLOC(F066): goto rom_F06A + (s16)(D1 & 0xffff); // JMP	2(PC,D1)
  DEF_ROMLOC(F066) : switch (switchindex) {
  case 0:
    rom_F06E();
    break;
  case 1:
...
ROMFUNC(rom_F06E) {
  ...
  DEF_ROMLOC(F072)
      : move_tomem_32(0xF18C, A0 + 0x4);             // MOVE.L	#$0000F18C,4(A0)
  ...
  DEF_ROMLOC(F092) : move_tomem_8(0xFFFFFFD7, A0 + 0x20); // MOVE.B
                                                          // #$D7,32(A0)
...
ROMFUNC(rom_F098) {
  ...
  DEF_ROMLOC(F0B4) : sub_toreg_16(read_16(A1 + 0xC), &D2); // SUB.W
                                                           // 12(A1),D2
  ...
  DEF_ROMLOC(F0F4) : move_toreg_16(SFX_BUMPER, &D0);             // MOVE.W	#$00B4,D0
  DEF_ROMLOC(F0F8) : play_sound_special();                 // JSR	$00001394
  ...
  DEF_ROMLOC(F10A)
      : cmp_tomem_8(0xFFFFFF8A,
                    (s32)A2 + (s8)0x2 + (s16)D0); // CMPI.B	#$8A,2(A2,D0)
  DEF_ROMLOC(F110) : if (CCR_CC) goto rom_F13C;   // BCC.B	$F13C
  DEF_ROMLOC(F112)
      : add_tomem_8(0x1,
                    (s32)A2 + (s8)0x2 + (s16)D0);  // ADDQ.B	#$01,2(A2,D0)
  ...
  DEF_ROMLOC(F124) : move_tomem_8(OBJ_POINTS, A1 + offsetof(object, id)); // MOVE.B	#$29,0(A1)
  DEF_ROMLOC(F12A)
