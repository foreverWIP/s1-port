...
ROMFUNC(rom_9370) {
	...
	DEF_ROMLOC(9372): move_toreg_8(read_8(A0 + 0x24), &D0); switchindex = D0 / 2; // MOVE.B	36(A0),D0
	DEF_ROMLOC(9376): move_toreg_16(read_16(0x937E + (s16)(D0 & 0xffff)), &D1); // MOVE.W	6(PC,D0),D1
	// DEF_ROMLOC(937A): goto rom_937E + (s16)(D1 & 0xffff); // JMP	2(PC,D1)
	DEF_ROMLOC(937A):
		switch (switchindex) {
			case 0: rom_9382(); break;
			case 1: rom_93C2(); break;
		}
}
ROMFUNC(rom_9382) {
	DEF_ROMLOC(9382): add_tomem_8(0x2, A0 + 0x24); // ADDQ.B	#$02,36(A0)
	DEF_ROMLOC(9386): move_tomem_32(0x9524, A0 + 0x4); // MOVE.L	#$00009524,4(A0)
...
ROMFUNC(rom_93C2) {
	...
	DEF_ROMLOC(93D8): if (CCR_EQ) {rom_DCCE();return;} // BEQ.W	$DCCE
