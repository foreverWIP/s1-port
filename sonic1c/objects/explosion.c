...
ROMFUNC(rom_93E0) {
  ...
  DEF_ROMLOC(93E2) : move_toreg_8(read_8(A0 + 0x24), &D0);
  switchindex = D0 / 2; // MOVE.B	36(A0),D0
  DEF_ROMLOC(93E6)
      : move_toreg_16(read_16(0x93EE + (s16)(D0 & 0xffff)),
                      &D1); // MOVE.W	6(PC,D0),D1
  // DEF_ROMLOC(93EA): goto rom_93EE + (s16)(D1 & 0xffff); // JMP	2(PC,D1)
  DEF_ROMLOC(93EA) : switch (switchindex) {
  case 0:
    rom_93F4();
    break;
  case 1:
...
ROMFUNC(rom_93F4) {
  ...
  DEF_ROMLOC(93FC) : if (!CCR_EQ) {
    rom_9416();
    return;
  }                                                // BNE.B	$9416
  DEF_ROMLOC(93FE) : move_tomem_8(OBJ_ANIMALS, A1 + offsetof(object, id)); // MOVE.B	#$28,0(A1)
  DEF_ROMLOC(9404)
      : move_tomem_16(read_16(A0 + 0x8), A1 + 0x8); // MOVE.W	8(A0),8(A1)
  DEF_ROMLOC(940A)
      : move_tomem_16(read_16(A0 + 0xC), A1 + 0xC); // MOVE.W	12(A0),12(A1)
  DEF_ROMLOC(9410)
      : move_tomem_16(read_16(A0 + 0x3E), A1 + 0x3E); // MOVE.W	62(A0),62(A1)
  rom_9416();
}
ROMFUNC(rom_9416) {
  DEF_ROMLOC(9416) : add_tomem_8(0x2, A0 + 0x24); // ADDQ.B	#$02,36(A0)
  DEF_ROMLOC(941A)
      : move_tomem_32(0x9544, A0 + 0x4);             // MOVE.L	#$00009544,4(A0)
...
ROMFUNC(rom_9474) {
  ...
  DEF_ROMLOC(9476) : move_toreg_8(read_8(A0 + 0x24), &D0);
  switchindex = D0 / 2; // MOVE.B	36(A0),D0
  DEF_ROMLOC(947A)
      : move_toreg_16(read_16(0x9482 + (s16)(D0 & 0xffff)),
                      &D1); // MOVE.W	6(PC,D0),D1
  // DEF_ROMLOC(947E): goto rom_9482 + (s16)(D1 & 0xffff); // JMP	2(PC,D1)
  DEF_ROMLOC(947E) : switch (switchindex) {
  case 0:
    rom_9486();
    break;
  case 1:
...
ROMFUNC(rom_9486) {
  ...
  DEF_ROMLOC(948A)
      : move_tomem_32(0x958A, A0 + 0x4);             // MOVE.L	#$0000958A,4(A0)
