...
ROMFUNC(rom_1A57E) {
  ...
  DEF_ROMLOC(1A5DC)
      : move_tomem_8(OBJ_BOSSPLASMA,
                     A1 + offsetof(object, id));            // MOVE.B	#$86,(A1)
  ...
  DEF_ROMLOC(1A5F8)
      : move_tomem_8(OBJ_EGGMANCYLINDER,
                     A1 + offsetof(object, id));    // MOVE.B	#$84,(A1)
...
ROMFUNC(rom_1A658) {
  ...
  DEF_ROMLOC(1A728)
      : move_tomem_8(0x64, A0 + 0x35); // MOVE.B	#$64,53(A0)
  DEF_ROMLOC(1A72E)
  ...
  DEF_ROMLOC(1A750) : animate_sprite();
  return;                                          // JMP	$0000ADA2
...
ROMFUNC(rom_1A7D6) {
  DEF_ROMLOC(1A7D6)
      : move_toreg_16(SFX_ELECTRIC, &D0); // MOVE.W
                                          // #$00B1,D0
...
ROMFUNC(rom_1A8C8) {
  ...
  DEF_ROMLOC(1A8FC)
      : or_toreg_16(read_16(A0 + 0x12), &D0); // OR.W
                                              // 18(A0),D0
...
ROMFUNC(rom_1A912) {
  ...
  DEF_ROMLOC(1A916) : animate_sprite();            // JSR	$0000ADA2
  rom_1A91C();
}
ROMFUNC(rom_1A91C) {
  DEF_ROMLOC(1A91C)
      : cmp_tomem_16(0x2700, v_limitright2);            // CMPI.W
                                                        // #$2700,$F72A
  DEF_ROMLOC(1A922) : if (CCR_GE) goto rom_1A928;       // BGE.B	$1A928
  DEF_ROMLOC(1A924) : add_tomem_16(0x2, v_limitright2); // ADDQ.W
                                                        // #$02,$F72A
...
ROMFUNC(rom_1A98A) {
  ...
  DEF_ROMLOC(1A9A2)
      : move_tomem_16(0x1E, A0 + 0x30); // MOVE.W	#$001E,48(A0)
  DEF_ROMLOC(1A9A8)
      : move_toreg_16(SFX_HITBOSS, &D0);            // MOVE.W
                                                    // #$00AC,D0
  ...
  DEF_ROMLOC(1A9BE)
      : move_tomem_16(0x60, A0 + 0x12); // MOVE.W	#$0060,18(A0)
  goto rom_1A9CC;
  return;                                           // BRA.B	$1A9CC
  ...
  DEF_ROMLOC(1AA0C) : v_gamemode = GM_ENDING;
  rom_1A502();
  return; // BRA.W	$1A502
...
ROMFUNC(rom_1AA1A) {
  ...
  DEF_ROMLOC(1AA46) : animate_sprite();             // JSR	$0000ADA2
...
ROMFUNC(rom_1AA7C) {
  ...
  DEF_ROMLOC(1AABE) : animate_sprite();               // JSR	$0000ADA2
  rom_1AA4C();
  return;                                  // BRA.W	$1AA4C
  ...
  DEF_ROMLOC(1AAF2) : animate_sprite();               // JSR	$0000ADA2
