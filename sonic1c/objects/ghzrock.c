...
ROMFUNC(rom_D6DE) {
	...
	DEF_ROMLOC(D6E0): move_toreg_8(read_8(A0 + 0x24), &D0); switchindex = D0 / 2; // MOVE.B	36(A0),D0
	DEF_ROMLOC(D6E4): move_toreg_16(read_16(0xD6EC + (s16)(D0 & 0xffff)), &D1); // MOVE.W	6(PC,D0),D1
	// DEF_ROMLOC(D6E8): goto rom_D6EC + (s16)(D1 & 0xffff); // JMP	2(PC,D1)
	DEF_ROMLOC(D6E8):
		switch (switchindex) {
			case 0: rom_D6F0(); break;
			case 1: rom_D714(); break;
		}
}
ROMFUNC(rom_D6F0) {
...
ROMFUNC(rom_D714) {
	...
	DEF_ROMLOC(D734): move_toreg_16(read_16(0xFFFFF700), &D1); // MOVE.W	$F700,D1
	...
	DEF_ROMLOC(D742): cmp_toreg_16(0x280, &D0); // CMPI.W	#$0280,D0
	DEF_ROMLOC(D746): if (CCR_HI) {rom_DCCE();return;} // BHI.W	$DCCE
