...
ROMFUNC(rom_F6EA) {
  ...
  DEF_ROMLOC(F6EC) : move_toreg_8(read_8(A0 + 0x24), &D0);
  switchindex = D0 / 2; // MOVE.B	36(A0),D0
  DEF_ROMLOC(F6F0)
      : move_toreg_16(read_16(0xF6F8 + (s16)(D0 & 0xffff)),
                      &D1); // MOVE.W	6(PC,D0),D1
  // DEF_ROMLOC(F6F4): goto rom_F6F8 + (s16)(D1 & 0xffff); // JMP	2(PC,D1)
  DEF_ROMLOC(F6F4) : switch (switchindex) {
  case 0:
    rom_F702();
    break;
  case 1:
...
ROMFUNC(rom_F702) {
  ...
  DEF_ROMLOC(F712) : move_tomem_8(OBJ_LAVAWALL, A1 + offsetof(object, id)); // MOVE.B	#$4E,0(A1)
  DEF_ROMLOC(F718)
      : move_tomem_32(0xFBBA, A1 + 0x4);              // MOVE.L	#$0000FBBA,4(A1)
  ...
  DEF_ROMLOC(F74A)
      : move_tomem_8(0xFFFFFF94, A1 + 0x20);       // MOVE.B	#$94,32(A1)
  DEF_ROMLOC(F750) : move_tomem_32(A0, A1 + 0x3C); // MOVE.L	A0,60(A1)
  DEF_ROMLOC(F754) : dec_reg_16(&D1);
  if ((D1 & 0xffff) != 0xffff)
    goto rom_F70C;                                 // DBF.W	D1,$F70C
...
ROMFUNC(rom_F762) {
  ...
  DEF_ROMLOC(F78C) : rom_F79E();
  return;                                  // BRA.B	$F79E
  DEF_ROMLOC(F78E) : tst_mem_8(A0 + 0x36); // TST.B	54(A0)
  DEF_ROMLOC(F792) : if (CCR_EQ) {
    rom_F79E();
    return;
  }                                                   // BEQ.B	$F79E
...
ROMFUNC(rom_F844) {
  ...
  DEF_ROMLOC(F846) : move_toreg_8(read_8(A0 + 0x24), &D0);
  switchindex = D0 / 2; // MOVE.B	36(A0),D0
  DEF_ROMLOC(F84A)
      : move_toreg_16(read_16(0xF852 + (s16)(D0 & 0xffff)),
                      &D1); // MOVE.W	6(PC,D0),D1
  // DEF_ROMLOC(F84E): goto rom_F852 + (s16)(D1 & 0xffff); // JMP	2(PC,D1)
  DEF_ROMLOC(F84E) : switch (switchindex) {
  case 0:
    rom_F85A();
    break;
  case 1:
...
ROMFUNC(rom_F85A) {
  ...
  DEF_ROMLOC(F864)
      : move_tomem_8(read_8(0xF856 + (s16)(D0 & 0xffff)),
                     A0 + 0x20); // MOVE.B	-16(PC,D0),32(A0)
  DEF_ROMLOC(F86A)
      : move_tomem_32(0xF89C, A0 + 0x4); // MOVE.L	#$0000F89C,4(A0)
  DEF_ROMLOC(F872) : move_tomem_8(0xFFFFFF84, A0 + 0x1); // MOVE.B
                                                         // #$84,1(A0)
