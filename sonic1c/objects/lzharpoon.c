...
ROMFUNC(rom_125D8) {
  ...
  DEF_ROMLOC(125DA) : move_toreg_8(read_8(A0 + 0x24), &D0);
  switchindex = D0 / 2; // MOVE.B	36(A0),D0
  DEF_ROMLOC(125DE)
      : move_toreg_16(read_16(0x125E6 + (s16)(D0 & 0xffff)),
                      &D1); // MOVE.W	6(PC,D0),D1
  // DEF_ROMLOC(125E2): goto rom_125E6 + (s16)(D1 & 0xffff); // JMP	2(PC,D1)
  DEF_ROMLOC(125E2) : switch (switchindex) {
  case 0:
    rom_125EC();
    break;
  case 1:
...
ROMFUNC(rom_125EC) {
  ...
  DEF_ROMLOC(125F0)
      : move_tomem_32(0x1266E, A0 + 0x4);             // MOVE.L	#$0001266E,4(A0)
...
ROMFUNC(rom_1261C) {
  ...
  DEF_ROMLOC(1262C)
      : move_tomem_8(read_8(0x12636 + (s16)(D0 & 0xffff)),
                     A0 + 0x20); // MOVE.B	8(PC,D0),32(A0)
  DEF_ROMLOC(12632) : rom_FD38();
  return; // BRA.W	$FD38
  DEF_ROMLOC(12636)
      : sub_tomem_32(D5, (A4 += 4, A4 - 4)); // SUB.L	D5,(A4)+
  DEF_ROMLOC(12638)
      : sub_tomem_32(D6, (A6 += 4, A6 - 4));     // SUB.L	D6,(A6)+
