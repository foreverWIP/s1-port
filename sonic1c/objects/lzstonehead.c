...
ROMFUNC(rom_128BC) {
  ...
  DEF_ROMLOC(128BE) : move_toreg_8(read_8(A0 + 0x24), &D0);
  switchindex = D0 / 2; // MOVE.B	36(A0),D0
  DEF_ROMLOC(128C2)
      : move_toreg_16(read_16(0x128CE + (s16)(D0 & 0xffff)),
                      &D1); // MOVE.W	10(PC,D0),D1
  // DEF_ROMLOC(128C6): rom_128CE + (s16)(D1 & 0xffff)(); // JSR	6(PC,D1)
  DEF_ROMLOC(128C6) : switch (switchindex) {
  case 0:
    rom_128DE();
    break;
  case 1:
  ...
  DEF_ROMLOC(128CA) : rom_FD38(); // BRA.W	$FD38
}
ROMFUNC(rom_128DE) {
...
ROMFUNC(rom_1291C) {
  ...
  DEF_ROMLOC(12934) : move_tomem_8(OBJ_GARGOYLE, A1 + offsetof(object, id)); // MOVE.B	#$62,0(A1)
