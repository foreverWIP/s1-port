...
ROMFUNC(rom_B542) {
  ...
  DEF_ROMLOC(B544) : move_toreg_8(read_8(A0 + 0x24), &D0);
  switchindex = D0 / 2; // MOVE.B	36(A0),D0
  DEF_ROMLOC(B548)
      : move_toreg_16(read_16(0xB550 + (s16)(D0 & 0xffff)),
                      &D1); // MOVE.W	6(PC,D0),D1
  // DEF_ROMLOC(B54C): goto rom_B550 + (s16)(D1 & 0xffff); // JMP	2(PC,D1)
  DEF_ROMLOC(B54C) : switch (switchindex) {
  case 0:
    rom_B560();
    break;
  case 1:
...
ROMFUNC(rom_B560) {
  ...
  DEF_ROMLOC(B564)
      : move_tomem_32(0xB95E, A0 + 0x4); // MOVE.L	#$0000B95E,4(A0)
  DEF_ROMLOC(B56C)
      : move_tomem_16(0xFFFFC000, A0 + 0x2);       // MOVE.W	#$C000,2(A0)
  ...
  DEF_ROMLOC(B596)
      : move_toreg_32(0xB554 + (s16)(D0 & 0xffff),
                      &A1); // LEA.L	-68(PC,D0),A1
  DEF_ROMLOC(B59A)
...
ROMFUNC(rom_B628) {
  ...
  DEF_ROMLOC(B634)
      : move_toreg_16(read_16(0xB63C + (s16)(D0 & 0xffff)),
                      &D1); // MOVE.W	6(PC,D0),D1
  // DEF_ROMLOC(B638): goto rom_B63C + (s16)(D1 & 0xffff); // JMP	2(PC,D1)
  DEF_ROMLOC(B638) : switch (switchindex) {
  case 0:
    rom_B648();
    break;
  case 1:
...
ROMFUNC(rom_B670) {
  ...
  DEF_ROMLOC(B67C)
      : move_toreg_16(read_16(A0 + 0x2C), &D1);   // MOVE.W	44(A0),D1
...
ROMFUNC(rom_B688) {
  ...
  DEF_ROMLOC(B6D6)
      : move_tomem_8(OBJ_GRASSFIRE,
                     A1 + offsetof(object, id)); // MOVE.B	#$35,0(A1)
  DEF_ROMLOC(B6DC)
      : move_tomem_16(read_16(A0 + 0x8), A1 + 0x8); // MOVE.W	8(A0),8(A1)
  DEF_ROMLOC(B6E2)
      : move_tomem_16(read_16(A0 + 0x2C), A1 + 0x2C); // MOVE.W	44(A0),44(A1)
  ...
  DEF_ROMLOC(B6F6)
      : move_tomem_32(read_32(A0 + 0x30), A1 + 0x30); // MOVE.L	48(A0),48(A1)
  ...
  DEF_ROMLOC(B720) : dec_reg_16(&D2);
  if ((D2 & 0xffff) != 0xffff)
    goto rom_B710;           // DBF.W	D2,$B710
...
ROMFUNC(rom_B726) {
  ...
  DEF_ROMLOC(B730)
      : move_toreg_32((s32)A2 + (s8)0x1 + (s16)D0,
                      &A2);                         // LEA.L	1(A2,D0),A2
...
ROMFUNC(rom_B744) {
  ...
  DEF_ROMLOC(B750)
      : move_toreg_16(read_16(A0 + 0x2A), &D0);     // MOVE.W	42(A0),D0
...
ROMFUNC(rom_B772) {
  ...
  DEF_ROMLOC(B790) : dec_reg_16(&D2);
  if ((D2 & 0xffff) != 0xffff)
    goto rom_B77E;                                 // DBF.W	D2,$B77E
