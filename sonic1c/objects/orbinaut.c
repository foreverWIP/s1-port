...
ROMFUNC(rom_123A2) {
  ...
  DEF_ROMLOC(123A4) : move_toreg_8(read_8(A0 + 0x24), &D0);
  switchindex = D0 / 2; // MOVE.B	36(A0),D0
  DEF_ROMLOC(123A8)
      : move_toreg_16(read_16(0x123B0 + (s16)(D0 & 0xffff)),
                      &D1); // MOVE.W	6(PC,D0),D1
  // DEF_ROMLOC(123AC): goto rom_123B0 + (s16)(D1 & 0xffff); // JMP	2(PC,D1)
  DEF_ROMLOC(123AC) : switch (switchindex) {
  case 0:
    rom_123BA();
    break;
  case 1:
...
ROMFUNC(rom_123BA) {
  DEF_ROMLOC(123BA)
      : move_tomem_32(0x125B8, A0 + 0x4);             // MOVE.L	#$000125B8,4(A0)
  DEF_ROMLOC(123C2) : move_tomem_16(0x429, A0 + 0x2); // MOVE.W	#$0429,2(A0)
  DEF_ROMLOC(123C8) : cmp_tomem_8(ZONE_SBZ, v_zone);  // CMPI.B	#$05,$FE10
  DEF_ROMLOC(123CE) : if (CCR_EQ) goto rom_123D6;     // BEQ.B	$123D6
  DEF_ROMLOC(123D0)
      : move_tomem_16(0x2429, A0 + 0x2);              // MOVE.W
                                                      // #$2429,2(A0)
  DEF_ROMLOC(123D6) : cmp_tomem_8(ZONE_LZ, v_zone);   // CMPI.B	#$01,$FE10
  ...
  DEF_ROMLOC(12460) : dec_reg_16(&D1);
  if ((D1 & 0xffff) != 0xffff)
    goto rom_12408;                                 // DBF.W	D1,$12408
  ...
  DEF_ROMLOC(12474)
      : move_tomem_8(read_8(A0 + 0x28), A0 + 0x24); // MOVE.B	40(A0),36(A0)
...
ROMFUNC(rom_12492) {
  ...
  DEF_ROMLOC(124B6) : tst_mem_16(v_debuguse);       // TST.W	$FE08
...
ROMFUNC(rom_124D4) {
  ...
  DEF_ROMLOC(1252A) : rom_DCCE(); // BRA.W	$DCCE
}
ROMFUNC(rom_1252E) {
  DEF_ROMLOC(1252E)
      : move_toreg_32(read_32(A0 + 0x3C), &A1); // MOVEA.L	60(A0),A1
  DEF_ROMLOC(12532)
      : cmp_tomem_8(OBJ_ORBINAUT,
                    A1 + offsetof(object, id)); // CMPI.B	#$60,0(A1)
  ...
  DEF_ROMLOC(12586)
      : add_toreg_16(read_16(A1 + 0xC), &D0);      // ADD.W	12(A1),D0
