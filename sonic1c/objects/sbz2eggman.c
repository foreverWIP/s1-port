...
ROMFUNC(rom_19FE2) {
  ...
  DEF_ROMLOC(19FE4) : move_toreg_8(read_8(A0 + 0x24), &D0);
  switchindex = D0 / 2; // MOVE.B	36(A0),D0
  DEF_ROMLOC(19FE8)
      : move_toreg_16(read_16(0x19FF0 + (s16)(D0 & 0xffff)),
                      &D1); // MOVE.W	6(PC,D0),D1
  // DEF_ROMLOC(19FEC): goto rom_19FF0 + (s16)(D1 & 0xffff); // JMP	2(PC,D1)
  DEF_ROMLOC(19FEC) : switch (switchindex) {
  case 0:
    rom_19FFC();
    break;
  case 1:
...
ROMFUNC(rom_19FFC) {
  ...
  DEF_ROMLOC(1A000) : move_tomem_16(0x2160, A0 + 0x8); // MOVE.W
                                                       // #$2160,8(A0)
  ...
  DEF_ROMLOC(1A022)
      : move_tomem_8(read_8(A2++), A0 + 0x24); // MOVE.B	(A2)+,36(A0)
  DEF_ROMLOC(1A026)
      : move_tomem_8(read_8(A2++), A0 + 0x1C); // MOVE.B	(A2)+,28(A0)
  DEF_ROMLOC(1A02A)
      : move_tomem_8(read_8(A2++), A0 + 0x18); // MOVE.B	(A2)+,24(A0)
  DEF_ROMLOC(1A02E)
      : move_tomem_32(0x1A1E4, A0 + 0x4);             // MOVE.L	#$0001A1E4,4(A0)
  ...
  DEF_ROMLOC(1A05E) : move_tomem_16(0x2130, A1 + 0x8); // MOVE.W
                                                       // #$2130,8(A1)
  ...
  DEF_ROMLOC(1A07A)
      : move_tomem_32(0xC52E, A1 + 0x4);              // MOVE.L	#$0000C52E,4(A1)
...
ROMFUNC(rom_1A0A0) {
  ...
  DEF_ROMLOC(1A0A2) : move_toreg_8(read_8(A0 + 0x25), &D0);
  switchindex = D0 / 2; // MOVE.B	37(A0),D0
  DEF_ROMLOC(1A0A6)
      : move_toreg_16(read_16(0x1A0BE + (s16)(D0 & 0xffff)),
                      &D1); // MOVE.W	22(PC,D0),D1
  // DEF_ROMLOC(1A0AA): rom_1A0BE + (s16)(D1 & 0xffff)(); // JSR
  // 18(PC,D1)
  DEF_ROMLOC(1A0AA) : switch (switchindex) {
  case 0:
    rom_1A0C6();
    break;
  case 1:
...
ROMFUNC(rom_1A106) {
  ...
  DEF_ROMLOC(1A10E)
      : move_tomem_16(0xFFFFFF04, A0 + 0x10); // MOVE.W	#$FF04,16(A0)
  DEF_ROMLOC(1A114)
      : move_tomem_16(0xFFFFFC40, A0 + 0x12);           // MOVE.W	#$FC40,18(A0)
  ...
  DEF_ROMLOC(1A13A) : move_tomem_16(0x5357, A0 + 0x28); // MOVE.W
                                                        // #$5357,40(A0)
  ...
  DEF_ROMLOC(1A166)
      : cmp_tomem_8(OBJ_FALSEFLOOR,
                    A1 + offsetof(object, id)); // CMPI.B	#$83,(A1)
  DEF_ROMLOC(1A16A) : if (!CCR_EQ) {
    dec_reg_16(&D0);
    if ((D0 & 0xffff) != 0xffff)
      goto rom_1A164;
  };                                                    // DBEQ.W	D0,$1A164
  DEF_ROMLOC(1A16E) : if (!CCR_EQ) goto rom_1A180;      // BNE.B	$1A180
  DEF_ROMLOC(1A170) : move_tomem_16(0x474F, A1 + 0x28); // MOVE.W
                                                        // #$474F,40(A1)
...
ROMFUNC(rom_1A184) {
  ...
  DEF_ROMLOC(1A186) : move_toreg_8(read_8(A0 + 0x25), &D0);
  switchindex = D0 / 2; // MOVE.B	37(A0),D0
  DEF_ROMLOC(1A18A)
      : move_toreg_16(read_16(0x1A192 + (s16)(D0 & 0xffff)),
                      &D0); // MOVE.W	6(PC,D0),D0
  // DEF_ROMLOC(1A18E): goto rom_1A192 + (s16)(D0 & 0xffff); // JMP	2(PC,D0)
  DEF_ROMLOC(1A18E) : switch (switchindex) {
  case 0:
    rom_1A196();
    break;
  case 1:
...
ROMFUNC(rom_1A196) {
  ...
  DEF_ROMLOC(1A19A) : cmp_tomem_16(0x5357, A1 + 0x28); // CMPI.W
                                                       // #$5357,40(A1)
  DEF_ROMLOC(1A1A0) : if (!CCR_EQ) {
    rom_1A1AC();
    return;
  }                                                 // BNE.B	$1A1AC
