...
ROMFUNC(rom_1A310) {
	...
	DEF_ROMLOC(1A312): move_toreg_8(read_8(A0 + 0x24), &D0); switchindex = D0 / 2; // MOVE.B	36(A0),D0
	...
	DEF_ROMLOC(1A31A):
		switch (switchindex) {
			case 0: rom_1A32A(); break;
			case 1: rom_1A3AC(); break;
			case 2: rom_1A3E6(); break;
			case 3: rom_1A412(); break;
			case 4: rom_1A422(); break;
			case 5: rom_1A430(); break;
		}
}
ROMFUNC(rom_1A32A) {
...
ROMFUNC(rom_1A3BC) {
	...
	DEF_ROMLOC(1A3E0): rom_10062(); return; // JMP	$00010062
}
ROMFUNC(rom_1A3AC) {
	DEF_ROMLOC(1A3AC): cmp_tomem_16(0x474F, A0 + 0x28); // CMPI.W	#$474F,40(A0)
	DEF_ROMLOC(1A3B2): if (!CCR_EQ) {rom_1A3BC();return;} // BNE.B	$1A3BC
...
ROMFUNC(rom_1A3E6) {
	...
	DEF_ROMLOC(1A40E): if (CCR_EQ) {rom_1A412();return;} // BEQ.B	$1A412
	DEF_ROMLOC(1A410): rom_1A3BC(); return; // BRA.B	$1A3BC
}
ROMFUNC(rom_1A412) {
...
ROMFUNC(rom_1A422) {
	...
	DEF_ROMLOC(1A428): if (CCR_EQ) {rom_1A444();return;} // BEQ.B	$1A444
	DEF_ROMLOC(1A42A): rom_DC92(); return; // JMP	$0000DC92
}
ROMFUNC(rom_1A430) {
	DEF_ROMLOC(1A430): tst_mem_8(A0 + 0x1); // TST.B	1(A0)
	DEF_ROMLOC(1A434): if (CCR_PL) {rom_19FDC();return;} // BPL.W	$19FDC
	DEF_ROMLOC(1A438): rom_DC40(); // JSR	$0000DC40
	DEF_ROMLOC(1A43E): rom_DC92(); return; // JMP	$0000DC92
}
ROMFUNC(rom_1A444) {
	...
	DEF_ROMLOC(1A4A4): rom_DC92(); return; // JMP	$0000DC92
}