...
ROMFUNC(rom_F21E) {
  ...
  DEF_ROMLOC(F22E)
      : move_toreg_16(SFX_SIGNPOST, &D0);    // MOVE.W
                                             // #$00CF,D0
  DEF_ROMLOC(F232) : play_sound();           // JSR	$0000138E
  DEF_ROMLOC(F238) : clr_mem_8(f_timecount); // CLR.B	$FE1E
  DEF_ROMLOC(F23C)
      : move_tomem_16(read_16(v_limitright2),
                      v_limitleft2);              // MOVE.W	$F72A,$F728
...
ROMFUNC(rom_F248) {
  ...
  DEF_ROMLOC(F2A6)
      : add_toreg_16(read_16(A0 + 0xC), &D0);         // ADD.W
                                                      // 12(A0),D0
...
ROMFUNC(rom_F2E0) {
  DEF_ROMLOC(F2E0) : tst_mem_16(v_debuguse);        // TST.W	$FE08
...
ROMFUNC(rom_F316) {
  ...
  DEF_ROMLOC(F326) : clr_mem_8(f_timecount);         // CLR.B	$FE1E
