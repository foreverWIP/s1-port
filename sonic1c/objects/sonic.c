...
ROMFUNC(rom_4012) {
  ...
  DEF_ROMLOC(4036)
      : if (v_gamemode != GM_SPECIAL) goto rom_403A; // BNE.B	$403A
...
ROMFUNC(rom_133A2) {
  ...
  DEF_ROMLOC(133B0) : queue_sprite();               // JSR	$0000DC92
...
ROMFUNC(rom_13EC6) {
  ...
  DEF_ROMLOC(13EF4) : queue_sprite();                // JMP	$0000DC92
}
ROMFUNC(rom_13EFA) {
  DEF_ROMLOC(13EFA)
      : move_toreg_16(read_16(0xFFFFF72E), &D0); // MOVE.W	$F72E,D0
...
ROMFUNC(rom_13F36) {
  ...
  DEF_ROMLOC(13F4C) : queue_sprite(); // JMP	$0000DC92
}
ROMFUNC(rom_1B7AA) {
