...
ROMFUNC(rom_CFE4) {
  ...
  DEF_ROMLOC(CFE6) : move_toreg_8(read_8(A0 + 0x24), &D0);
  switchindex = D0 / 2; // MOVE.B	36(A0),D0
  DEF_ROMLOC(CFEA)
      : move_toreg_16(read_16(0xCFF2 + (s16)(D0 & 0xffff)),
                      &D1); // MOVE.W	6(PC,D0),D1
  // DEF_ROMLOC(CFEE): goto rom_CFF2 + (s16)(D1 & 0xffff); // JMP	2(PC,D1)
  DEF_ROMLOC(CFEE) : switch (switchindex) {
  case 0:
    rom_D002();
    break;
  case 1:
...
ROMFUNC(rom_D018) {
  ...
  DEF_ROMLOC(D02C)
      : move_toreg_8(read_8((s32)A3 + (s8)0x0 + (s16)D2),
                     &D3);                        // MOVE.B	0(A3,D2),D3
  ...
  DEF_ROMLOC(D03E)
      : move_tomem_32(0xD482, A1 + 0x4); // MOVE.L	#$0000D482,4(A1)
  DEF_ROMLOC(D046)
      : move_tomem_16(0xFFFF8541, A1 + 0x2);        // MOVE.W	#$8541,2(A1)
  DEF_ROMLOC(D04C) : move_tomem_8(0x0, A1 + 0x1);   // MOVE.B	#$00,1(A1)
  DEF_ROMLOC(D052) : move_toreg_32(A1 + 0x40, &A1); // LEA.L	64(A1),A1
  DEF_ROMLOC(D056) : dec_reg_16(&D1);
  if ((D1 & 0xffff) != 0xffff)
    goto rom_D018; // DBF.W	D1,$D018
