...
ROMFUNC(rom_A554) {
  ...
  DEF_ROMLOC(A556) : move_toreg_8(read_8(A0 + 0x24), &D0);
  switchindex = D0 / 2; // MOVE.B	36(A0),D0
  DEF_ROMLOC(A55A)
      : move_toreg_16(read_16(0xA562 + (s16)(D0 & 0xffff)),
                      &D1); // MOVE.W	6(PC,D0),D1
  // DEF_ROMLOC(A55E): goto rom_A562 + (s16)(D1 & 0xffff); // JMP	2(PC,D1)
  DEF_ROMLOC(A55E) : switch (switchindex) {
  case 0:
    rom_A568();
    break;
  case 1:
...
ROMFUNC(rom_A568) {
  ...
  DEF_ROMLOC(A58C)
      : move_tomem_8(0xFFFFFFFF, A0 + 0x1A); // MOVE.B
                                             // #$FF,26(A0)
