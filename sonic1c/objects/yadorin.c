...
ROMFUNC(rom_FEC6) {
  ...
  DEF_ROMLOC(FEC8) : move_toreg_8(read_8(A0 + 0x24), &D0);
  switchindex = D0 / 2; // MOVE.B	36(A0),D0
  DEF_ROMLOC(FECC)
      : move_toreg_16(read_16(0xFED4 + (s16)(D0 & 0xffff)),
                      &D1); // MOVE.W	6(PC,D0),D1
  // DEF_ROMLOC(FED0): goto rom_FED4 + (s16)(D1 & 0xffff); // JMP	2(PC,D1)
  DEF_ROMLOC(FED0) : switch (switchindex) {
  case 0:
    rom_FED8();
    break;
  case 1:
...
ROMFUNC(rom_FED8) {
  DEF_ROMLOC(FED8)
      : move_tomem_32(0xFFBA, A0 + 0x4);              // MOVE.L	#$0000FFBA,4(A0)
  ...
  DEF_ROMLOC(FF04)
      : move_tomem_8(0xFFFFFFCC, A0 + 0x20);        // MOVE.B	#$CC,32(A0)
...
ROMFUNC(rom_FF2C) {
  ...
  DEF_ROMLOC(FF2E) : move_toreg_8(read_8(A0 + 0x25), &D0);
  switchindex = D0 / 2; // MOVE.B	37(A0),D0
  DEF_ROMLOC(FF32)
      : move_toreg_16(read_16(0xFF48 + (s16)(D0 & 0xffff)),
                      &D1); // MOVE.W	20(PC,D0),D1
  // DEF_ROMLOC(FF36): rom_FF48 + (s16)(D1 & 0xffff)(); // JSR	16(PC,D1)
  DEF_ROMLOC(FF36) : switch (switchindex) {
  case 0:
    rom_FF4C();
    break;
  case 1:
