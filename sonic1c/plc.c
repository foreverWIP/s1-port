...
void add_plc(void) {
	create_plc_common(false);
}
void new_plc(void) {
	create_plc_common(true);
}
void clear_plc(void) {
...
void run_plc(void) {
  ...
  DEF_ROMLOC(160A) : move_tomem_16(D2, v_plc_patternsleft);     // MOVE.W	D2,$F6F8
...
void plc_pop_front(void) {
  ...
  DEF_ROMLOC(16DA) : move_tomem_32(read_32(A0 + 0x6),
                      A0); A0 += 4; // MOVE.L	6(A0),(A0)+
...
void quick_plc(void) {
  move_toreg_32(0x1DD86, &A1); // LEA.L	$0001DD86,A1
  add_toreg_16(D0, &D0);       // ADD.W	D0,D0
  move_toreg_16(read_16((s32)A1 + (s8)0x0 + (s16)D0), &D0); // MOVE.W	0(A1,D0),D0
