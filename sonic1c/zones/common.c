...
ROMFUNC(rom_5ECC) {
  ...
  DEF_ROMLOC(5F1A) : move_tomem_16(0x1010, 0xFFFFF74A); // MOVE.W
                                                        // #$1010,$F74A
  DEF_ROMLOC(5F20)
      : move_toreg_16(read_16((A0 += 2, A0 - 2)), &D0); // MOVE.W	(A0)+,D0
  ...
  DEF_ROMLOC(60E6)
      : move_toreg_16(read_16(0xFFFFF72A), &D2);    // MOVE.W	$F72A,D2
...
ROMFUNC(rom_737C) {
  ...
  DEF_ROMLOC(73A6) : move_tomem_8(read_8(A1++), A0++); // MOVE.B
                                                       // (A1)+,(A0)+
